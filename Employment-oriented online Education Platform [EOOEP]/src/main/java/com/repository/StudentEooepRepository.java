...
public class StudentEooepRepository {
    ...
    public StudentEooep getByStudentEooepname(String name) {
        Session session = sessionFactory.getCurrentSession();
        Query<StudentEooep> studentEooepQuery = session.createQuery("from StudentEooep where name = :name", StudentEooep.class);
        studentEooepQuery.setParameter("name", name);
